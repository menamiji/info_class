...
void main() {
  ...
  group('JWT Authentication Integration Tests', () {
    ...
    testWidgets('2. AuthService Basic Functionality', (
      WidgetTester tester,
    ) async {
      // Test AuthService currentUser getter
      final initialUser = AuthService.currentUser;
      expect(initialUser, isNull, reason: 'User should be null initially');
    ...
    testWidgets('3. TokenStorage Functionality', (WidgetTester tester) async {
      ...
      expect(initialToken, isNull, reason: 'No token should exist initially');
    ...
    testWidgets('4. ApiClient Firebase Token Exchange (Mock)', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('5. ApiClient getCurrentUser (Unauthenticated)', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('6. AuthNotifier State Management', (
      WidgetTester tester,
    ) async {
      ...
      expect(initialState.value, isNull, reason: 'Initial user should be null');

      // Test authentication status
      final isAuth = container.read(isAuthenticatedProvider);
      expect(isAuth, isFalse, reason: 'Should not be authenticated initially');

      final isLoading = container.read(isAuthLoadingProvider);
      expect(isLoading, isFalse, reason: 'Should not be loading initially');

      container.dispose();
    });

    testWidgets('7. Complete Authentication Flow (Mock)', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('8. Error Handling in AuthNotifier', (
      WidgetTester tester,
    ) async {
      final container = ProviderContainer();
  ...
  group('Edge Cases and Error Scenarios', () {
    ...
    testWidgets('Malformed Data Handling', (WidgetTester tester) async {
      ...
      expect(response.ok, isFalse, reason: 'Empty token should be rejected');
      expect(
        response.error,
        isNotNull,
